...
public class ContestService{
    ...
    public Contestant createContestant(Long contestId, String userName) {
        Contest c = contestRepository.findById(contestId).orElseThrow(() -> new RuntimeException());
        User u = userRepository.findByName(userName).orElseThrow(() -> new RuntimeException());
        if(ContestStatus.IN_PROGRESS.equals(c.getContestStatus())){
            throw new RuntimeException("Contest has already started!");
        }
        if(ContestStatus.ENDED.equals(c.getContestStatus())){
            throw new RuntimeException("Contest has already ended!");
        }


     return contestantRepository.save(new Contestant(u, c));
    }
    ...
    public List<Contestant> contestHistory(Long contestId) {
        ...
        if(contest.getContestStatus().equals(ContestStatus.NOT_STARTED)){
            throw new RuntimeException("Contest Not Yet Started!");
        }
        ...
        Collections.sort(contestants,(a,b)-> b.getCurrentContestPoints() - a.getCurrentContestPoints());
        return contestants;
    }  
}
